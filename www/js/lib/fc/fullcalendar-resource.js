...
function Header(calendar, options) {
	...
	function render() {
	    /* ricky: custom unitrak toolbar */
	    var unitrak_users = '<div class="btn-group">' +
            '<button type="button" class="btn btn-default" >Staff</button>' +
            '<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">' +
            '<span class="caret"></span>' +
            '<span class="sr-only">Toggle Dropdown</span>' +
            '</button><ul class="dropdown-menu" role="menu" id="unitrak-calendar-users">' +
            '</ul></div></div>';
		tm = options.theme ? 'ui' : 'fc';
  ...
  // exports
	...
	function buildDayTableHeadHTML() {
		...
		for (col=0; col<colCnt; col++) {
	      ...
	      html +=
					"<th class='" + classNames.join(' ') + "'><a onclick='_go_resource(" + resource.id + ")' style=' color:#000'>" +
...
function ResourceEventRenderer() {
	...
	function slotSegHtml(event, seg) {
	    var _color = "#fff";
	    var _timestring =
            "<div class='fc-event-time' style='font-weight: bold; font-size:11px;'>" +
			htmlEscape(t.getEventTimeText(event)) + " " + event.location +
			"</div>";
			
	    var _invoicelink =
			"<div class='fc-event-title' style='font-weight: bold; font-size:11px;'>" +
			htmlEscape(event.title || '') +
			"</div>";		
	    ...
	    if (event.type == "lock")
	    {
	        ...
	        // _timestring =
                // "<div class='fc-event-time'><a href='Job.aspx?id=" + event.jobID + "&tid=" + event.techid + "&redi=schedule' style=' color:#000'>" +
                // htmlEscape(t.getEventTimeText(event)) + " " + event.location +
                // "</a></div>";
			
	        if (event.recurrenceID == 1) {
                    _timestring =
                        "<div class='fc-event-time'><a href='Job.aspx?id=" + event.jobID + "&tid=" + event.techid + "&redi=schedule' style=' color:#000; font-weight: bold; font-size:11px;'>" +
                        htmlEscape(t.getEventTimeText(event)) + " " + event.location + 
                    "</a> <i style=' color:#000' class='fa fa-refresh'></i></div>";
                }
                else {
                    _timestring =
                        "<div class='fc-event-time'><a href='Job.aspx?id=" + event.jobID + "&tid=" + event.techid + "&redi=schedule' style=' color:#000; font-weight: bold; font-size:11px;'>" +
	        ...
	        var _timericon = "";
	        if (event.timerstatus == 0)//no timer
	        {
	            _timericon = " <i style=' color:#000' class='fa fa-play'></i>";
	        }
	        else if (event.timerstatus == 1)//timer end
	        {
	            _timericon = " <i style=' color:#000' class='fa fa-youtube-play'></i>";
	        }
	        else if (event.timerstatus == 2)//timer pause
	        {
	            _timericon = " <i style=' color:#000' class='fa fa-pause'></i>";
	        }
	        else if (event.timerstatus == 3)//timer running
	        {
	            _timericon = " <i style=' color:#000' class='fa fa-play-circle-o'></i>";
	        }
            ...
            if (event.recurrenceID == 1) {
                _timestring =
                    "<div class='fc-event-time'><a href='Job.aspx?id=" + event.jobID + "&tid=" + event.techid + "&redi=schedule' style=' color:#000; font-weight: bold; font-size:11px;'>" +
                    htmlEscape(t.getEventTimeText(event)) + " " + event.location + 
                "</a> <i style=' color:#000' class='fa fa-refresh'></i>" + _timericon + "</div>";
            }
            else {
                _timestring =
                    "<div class='fc-event-time'><a href='Job.aspx?id=" + event.jobID + "&tid=" + event.techid + "&redi=schedule' style=' color:#000; font-weight: bold; font-size:11px;'>" +
	        ...
	        if (event.withinvoice == "true") {
	            _invoicelink =
                    "<div class='fc-event-title'><a style=' color: #000; font-weight: bold; font-size:11px;' href='Job-invoices.aspx?id=" + event.jobID + "'>" +
                    htmlEscape(event.title || '') +
                    "</a></div>";
	        }
		...
		html +=
			...
			" style=" +
				...
				"position:absolute;" +
				"top:" + seg.top + "px;" +
				"left:" + seg.left + "px; background-color:" + _bgcolor + "; color:" + _color + 
				skinCss +
				"'" +
			">" +
			"<div class='fc-event-inner'>" +
